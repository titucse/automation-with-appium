...
class NPSBBeneficiaryCard {
    ...
    async selectBeneficiaryType() {
        ...
        const otherBankAccount = await $('~অন্য ব্যাংক কার্ড');
    ...
    async addBeneficiary() {
        ...
        const cardNumber = await $('android=new UiSelector().className("android.widget.EditText").instance(0)');
        // verify that the account number input field is displayed
        await cardNumber.isDisplayed();
        // Tap account number input field
        await cardNumber.click();
        // Enter account number
        const cardNo = data.npsbBeneficiarycard.cardno;
        // Enter account number
        await cardNumber.setValue(cardNo.toString());
        // wait for 3 seconds
        await driver.pause(1000);
        ...
        const cardName = data.npsbBeneficiarycard.nickname;
        // Enter nick name
        await NickName.setValue(cardName.toString());

        // Account holder name input field is visible
        const cardHolderName = await $('android=new UiSelector().className("android.widget.EditText").instance(2)');
        ...
        const cardholder = data.npsbBeneficiarycard.cardholdername;
        // Enter account holder name
        await cardHolderName.setValue(cardholder.toString());
        // wait for 3 seconds
        await driver.pause(1000);
        ...
        const mobileno = data.npsbBeneficiarycard.mobileno;
        // Enter mobile number
        await mobileNumber.setValue(mobileno.toString());
        // wait for 3 seconds
        await driver.pause(1000);
        // bank name input field is visible
        const bankName = await $('//android.widget.ScrollView/android.view.View[19]');
        // click on bank name
        ...
        const SelectbankName = await $('~AB BANK LTD.');
        // click on bank name
...
module.exports = new NPSBBeneficiaryCard();